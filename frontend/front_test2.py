...
# URL por defecto (Localhost o Producción)
# Si estás probando local, asegúrate de que sea http://127.0.0.1:8000/api/ask
...
st.markdown("""
    ...
    /* 3. Input Flotante (Ajustado a la columna derecha) */
    .stChatInput {
...
def get_api_response(message_text):
    payload = {
        ...
        "model": "llama-3" # Puedes cambiar esto si tu backend usa otro alias
    }
    ...
    try:
        ...
        # (Para poder leer el mensaje de error del backend si es un 422)
        try:
            return response.json()
        except:
            # Si no es JSON, lanzamos el error HTTP estándar
...
# INPUT FLOTANTE
prompt = st.chat_input("Ej: Ventas de Zapatillas vs Laptops...")
...
if prompt:
    ...
    # Verificamos si hay respuesta. Incluso si 'exito' es False, mostramos el mensaje.
    if resp:
        ...
        # Extraemos 'mensaje' del JSON. Si viene vacío, ponemos un default.
        bot_msg = resp.get("mensaje") 
        if not bot_msg:
             bot_msg = "Datos recibidos (Sin mensaje del servidor)."
        
        st.session_state.messages.append({"role": "assistant", "content": bot_msg})
        
    else:
        st.session_state.messages.append({"role": "assistant", "content": "❌ Error: No hubo respuesta del servidor."})
